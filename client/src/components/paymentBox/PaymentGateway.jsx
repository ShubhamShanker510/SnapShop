...
import { deleteAllCartDelete, getTotalPrice } from '../../hooks/apiCall';
...
const PaymentGateway = ({css}) => {
    ...
    const HandleSubmit=(e)=>{
        ...
        if(!creditCardRegex.test(cardNumber)){
            ...
            toast.error("Invalid Details", {autoClose: 3000, position: 'top-center'})
            valid=false;
        }
        ...
        if(valid){
            toast.success("Payment Successfull", {autoClose: 3000, position: 'top-center'})
            setTimeout(async()=>{
                navigate('/')
        ...
        <div className="header py-9 bg-red-700 text-white flex justify-center font-semibold p-2 text-[50px] rounded-t-2xl w-[500px]">
            ...
            <div className="sec2 mb-8">
                <div className="amount pt-3">
                    <p>Amount: Rs. <span className='text-green-600 font-bold'>{totalPrice}</span></p>
                </div>
